...
public class PaintTest {
	...
	public static void main(String[] args) {
		...
		scheduler.scheduleAtFixedRate(toRun, 0, 1, TimeUnit.SECONDS);
	}
	...
	private static List<IPaintable> createNewPattern() {
		...
		for (int i = 1; i <= 50; i++) {
			double x1 = (Math.random() * 49) + 1;
			double y1 = (Math.random() * 49) + 1;
			double x2 = (Math.random() * 49) + 1;
			double y2 = (Math.random() * 49) + 1;
